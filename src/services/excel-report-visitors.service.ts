...
export class ExcelReportVisitorsService {
  ...
  async generateExcelReport(dateFrom: string | null, dateTo: string | null, setLoading: (loading: boolean) => void) {
    ...
    const allVisitorData: any[] = [];
    let currentPage = 1;
    const pageSize = 10;

    const fetchAllPages = async (page: number) => {
      try {
        const response = await this.recordsService.getLogsReports('visitor', pageSize, page, dateFrom, dateTo).toPromise();
        const visitorData = response?.records || [];
        allVisitorData.push(...visitorData);

        if (visitorData.length === pageSize) {
          // If there are more records, fetch the next page
          currentPage++;
          await fetchAllPages(currentPage);
        }
      } catch (error) {
        console.error('Error fetching visitor data:', error);
        throw error;  // Rethrow the error to be handled in the outer try-catch
      }
    };
    ...
    try {
      ...
      // Add data rows
      allVisitorData.forEach((visitor: any) => {
        worksheet.addRow([
          visitor.school,
          visitor.name,
          visitor.time_in,
      ...
      const totalRecords = `Total Visitor Records: ${allVisitorData.length}`;
