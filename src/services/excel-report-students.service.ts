...
export class ExcelReportStudentsService {
  ...
  async generateExcelReport(dateFrom: string | null, dateTo: string | null, setLoading: (loading: boolean) => void) {
    ...
    const fetchAllPages = async (page: number) => {
      try {
        const response = await this.recordsService.getLogsReports('student', pageSize, page, dateFrom, dateTo).toPromise();
        const studentData = response?.records || [];
        allStudentData.push(...studentData);

        if (studentData.length === pageSize) {
    ...
    try {
      ...
      const totalRecords = `Total Student Records: ${allStudentData.length}`;
