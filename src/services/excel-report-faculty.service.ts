...
export class ExcelReportFacultyService {
  ...
  async generateExcelReport(dateFrom: string | null, dateTo: string | null, 
    ...
    try {
      ...
      const totalRecords = `Total Faculty Records: ${allFacultyData.length}`;
  ...
  private async fetchAllFacultyLogs(dateFrom: string | null, dateTo: string | null): Promise<any[]> {
    ...
    while (hasMoreRecords) {
      const response = await this.recordsService.getLogsReports('faculty', pageSize, page, dateFrom, dateTo).toPromise();
