...
try {
    $firstName = isset($firstName) ? ucwords(strtolower(trim($firstName))) : null;
    $lastName = isset($lastName) ? ucwords(strtolower(trim($lastName))) : null;
    $email = isset($email) ? strtolower(trim($email)) : null;

    $stmt = $conn->prepare(
        "INSERT INTO users (user_type, created_at, updated_at) 
         VALUES (?, NOW(), NOW())"
    );
    ...
    if ($selectedRole == 'student') {
        ...
        $stmt = $conn->prepare(
            "INSERT INTO students (
                user_id, student_number, first_name, surname, gender, 
                phone_number, course_id, email, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())"
        );
        $stmt->bind_param(
            "isssssss", $userId, $studentNumber, $firstName, $lastName, $sex, 
            $contact, $courseId, $email
        );
        ...
        $stmt = $conn->prepare(
            "INSERT INTO faculty (
                user_id, emp_number, first_name, surname, gender, phone_number, 
                dept_id, email, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())"
        );
        $stmt->bind_param(
        ...
        $stmt = $conn->prepare(
            "INSERT INTO visitor (
                user_id, first_name, surname, gender, phone_number, school, 
                identifier, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())"
        );
        $stmt->bind_param(
            "issssss", $userId, $firstName, $lastName, $sex, $contact, $school, 
            $identifier
        );
    } else {
